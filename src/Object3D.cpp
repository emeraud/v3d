...
bool Object3D::intersect(const Ray& ray, Vec3Df& intersectionPoint, Vec3Df& intersectionNormal) const {
  ...
  for (std::vector<IntersectedNode>::const_iterator nIt = nodes.begin(); nIt != nodes.end(); ++nIt) {
    ...
    for (std::vector<const Vertex*>::const_iterator vIt = vertices.begin(); vIt != vertices.end(); ++vIt) {
      for (std::vector<Triangle*>::const_iterator tIt = (*vIt)->triangles.begin(); tIt != (*vIt)->triangles.end(); ++tIt) {
        v0 = _tessellation->getVertex((*tIt)->getVertex(0));
        v1 = _tessellation->getVertex((*tIt)->getVertex(1));
        v2 = _tessellation->getVertex((*tIt)->getVertex(2));
  ...
  if (minDist != FLT_MAX) {
    ...
    intersectionNormal = Vec3Df::crossProduct(i0->pos-i1->pos, i2->pos-i1->pos);
